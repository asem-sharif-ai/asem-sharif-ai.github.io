...
function initGuestbookModal() {
  ...
  overlay.addEventListener("click", (e) => {
    if (e.target === overlay) closeGuestbookModal();
  });
}
...
function renderFeed(feedList) {
  ...
  if (!Array.isArray(feedList) || feedList.length === 0) {
    ...
    renderNoData(
      "No Feed Yet - Check Back Soon For Updates",
      "list-container",
      false,
    );
    return;
    ...
    words.length > 0
      ? feedList.filter((item) => {
          return words.every((w) =>
            `${item.title} ${item.subtitle} ${item.date} ${formatDuration(item.date)}`
              .toLowerCase()
              .includes(w),
          );
        })
  ...
  const totalPages = Math.max(1, Math.ceil(sorted.length / _pageLength.feed));
  if (_pageIndex.feed > totalPages) _pageIndex.feed = totalPages;
  if (_pageIndex.feed < 1) _pageIndex.feed = 1;

  const start = (_pageIndex.feed - 1) * _pageLength.feed;
  const pageItems = sorted.slice(start, start + _pageLength.feed);
  pageItems.forEach((item) => container.appendChild(buildFeedCard(item)));

  container.appendChild(
    buildPagerRow(_pageIndex.feed, totalPages, (newPage) => {
      _pageIndex.feed = newPage;
      _saveHubPageIndex();
      renderFeed(_allFeed);
    }),
  );
...
function buildFeedCard(item) {
  ...
  const dateLabel = item.date
    ? highlightText(`${item.date}${duration ? ` · ${duration}` : ""}`, query)
    : "";
  ...
  if (isAdmin) {
    msgPane
      .querySelector(".feed-btn-edit")
      ?.addEventListener("click", () => openAdminEditModal(item));
    msgPane
      .querySelector(".feed-btn-pin")
      ?.addEventListener("click", () => feedAdminTogglePin(item, card));
    msgPane
      .querySelector(".feed-btn-hide")
      ?.addEventListener("click", () => feedAdminToggleVisibility(item, card));
  }
  ...
  if (item.gallery && item.gallery.content.length > 0) {
    ...
    row.appendChild(
      buildGalleryPane(item.gallery.content, item.gallery.header),
    );
...
async function feedAdminToggleVisibility(item, cardUI) {
  try {
    const data = await fetchGuestbook("feed_hide", { id: item.id });
    const post = _allFeed.find((p) => p.id === item.id);
...
function applyReactUI(btnUI, liked, count) {
  ...
  if (icon)
    icon.className = `${liked ? "fa-solid reacted" : "fa-regular"} fa-heart`;
...
async function loadGuestbook() {
  if (_currentPage === "feed")
    renderNoData("Loading Posts", "list-container", false);
  if (_currentPage === "guests")
    renderNoData("Loading Guests", "list-container", false);
  ...
  try {
    ...
    if (_currentPage === "guests")
      renderNoData("Failed To Load Guestbook", "list-container", false);
    console.error(e);
  }
}
...
function renderGuestbook() {
  ...
  if (isAdmin) {
    ...
    const filteredPool = pool.filter((e) => {
      if (e.status === "banned") {
        return (
          words.length === 0 ||
          (e.id && e.id.toLowerCase().includes(rawQuery.toLowerCase()))
        );
      }
    ...
    finalBanned = filteredPool
      .filter((e) => e.status === "banned")
      .map((e) => ({ id: e.id, status: "banned" }));
  } else {
  ...
  if (isAdmin) {
    ...
    const sortedEntries = [...finalEntries].sort(
      (a, b) => (statusOrder[a.status] || 99) - (statusOrder[b.status] || 99),
    );
    orderedCards = [
      ...sortedEntries.map((e) => ({ entry: e, isAdmin: true, banned: false })),
      ...finalBanned.map((e) => ({ entry: e, isAdmin: false, banned: true })),
    ];
  } else {
    orderedCards = [
      ...finalEntries
      ...
      ...finalEntries
        .filter((e) => !e.pin)
        .sort((a, b) => (b.date > a.date ? 1 : -1)),
    ].map((e) => ({ entry: e, isAdmin: false, banned: false }));
  }

  const totalPages = Math.max(
    1,
    Math.ceil(orderedCards.length / _pageLength.guests),
  );
  if (_pageIndex.guests > totalPages) _pageIndex.guests = totalPages;
  if (_pageIndex.guests < 1) _pageIndex.guests = 1;

  const start = (_pageIndex.guests - 1) * _pageLength.guests;
  const pageCards = orderedCards.slice(start, start + _pageLength.guests);
  pageCards.forEach(({ entry, isAdmin: cardIsAdmin, banned }) =>
    container.appendChild(buildGuestbookCard(entry, cardIsAdmin, banned)),
  );

  container.appendChild(
    buildPagerRow(_pageIndex.guests, totalPages, (newPage) => {
      _pageIndex.guests = newPage;
      _saveHubPageIndex();
      renderGuestbook();
    }),
  );
...
function buildGuestbookCard(entry, isAdmin = false, banned = false) {
  if (banned) {
    ...
    `;
    card
      .querySelector(".feed-btn-unban")
      .addEventListener("click", () => gbAdminAction("unban", entry.id, card));
    return card;
  }
  ...
  const avatarUI = entry.image
    ? `<img class='feed-card-avatar' src='${entry.image}' alt='avatar' referrerpolicy='no-referrer' />`
  ...
  if (isAdmin) {
    const badgeClass =
      {
        new: "keyword feed-badge feed-badge-new",
  ...
  if (isAdmin) {
    card
      .querySelector(".feed-btn-approve")
      ?.addEventListener("click", () =>
        gbAdminAction("approve", entry.id, card),
      );
    card
      .querySelector(".feed-btn-heart")
      ?.addEventListener("click", () => gbAdminAction("like", entry.id, card));
    card
      .querySelector(".feed-btn-pin")
      ?.addEventListener("click", () => gbAdminAction("pin", entry.id, card));
    card
      .querySelector(".feed-btn-delete")
      ?.addEventListener("click", () =>
        gbAdminAction("delete_message", entry.id, card),
      );
    card
      .querySelector(".feed-btn-remove")
      ?.addEventListener("click", () =>
        gbAdminAction("remove", entry.id, card),
      );
    card
      .querySelector(".feed-btn-ban")
      .addEventListener("click", () => gbAdminAction("ban", entry.id, card));
  }
...
function setModalPage(state, editingPost = null) {
  ...
  if (state === "login") {
    ...
    const otpInputPlaceholder =
      _liveOTP.phase === "verify" ? "Enter The Code" : "Authorized Only";
  ...
  if (state === "admin") {
    ...
    const galleryImages =
      isEditing && editingPost.gallery?.content
        ? editingPost.gallery.content
        : [];
...
async function gbAdminAction(action, id, cardUI) {
  try {
    ...
    if (action === "remove") {
      if (_allGB) _allGB = _allGB.filter((e) => e.id !== id);
      cardUI.remove();
      return;
    }
    ...
    if (action === "approve") {
      ...
      if (btnIcon)
        btnIcon.className = `fa-solid ${data.status === "approved" ? "fa-eye" : "fa-eye-slash"}`;
      const badge = cardUI.querySelector(".feed-badge");
      if (badge) {
        badge.textContent = data.status === "approved" ? "Approved" : "Pending";
        badge.className =
          data.status === "approved"
            ? "keyword feed-badge feed-badge-approved"
            : "keyword feed-badge feed-badge-pending";
      }
    }
...
function modalHandlers(state, editingPost = null) {
  document
    .getElementById("feed-close-modal-btn")
  ...
  if (state === "admin") {
    ...
    const galleryHeaderInput = document.getElementById(
      "feed-admin-gallery-header",
    );
    const galleryPreview = document.getElementById(
      "feed-admin-gallery-preview",
    );

    document
      .getElementById("feed-unlink-btn")
    ...
    const renderGalleryPreview = () => {
      ...
      galleryPreview
        .querySelectorAll(".feed-admin-gallery-thumb")
        .forEach((thumb) => {
      ...
      galleryHeaderInput?.classList.toggle(
        "hub-hidden",
    ...
    document
      .getElementById("feed-admin-cancel-btn")
      ?.addEventListener("click", () => {
        ...
        if (contentInput)
          contentInput.value = Array.isArray(editingPost?.content)
    ...
    document
      .getElementById("feed-admin-delete-btn")
      ?.addEventListener("click", async () => {
    ...
    document
      .getElementById("feed-admin-save-btn")
      ?.addEventListener("click", async () => {
        ...
        try {
          ...
          for (const item of pendingGallery) {
            ...
            const res = await fetchGuestbook("gallery_upload", {
              data: base64,
          ...
          const gallery =
            finalImages.length > 0
              ? { header: galleryHeader, content: finalImages }
              : null;
          await fetchGuestbook("feed_save", {
          ...
          pendingGallery = finalImages.map((img) => ({
            existing: true,
            id: img.id,
            url: img.url,
          }));
  ...
  if (state === "login") {
    document
      .getElementById("feed-verify-btn")
      ?.addEventListener("click", async () => {
        setModalPage("loading");
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        ...
        if (_liveOTP.phase === "send") {
          ...
          try {
            ...
            localStorage.setItem(
              addresses.hubLiveOTP,
              JSON.stringify({
                email: _liveOTP.email,
                startAt: _liveOTP.startAt,
              }),
            );
            setModalPage("login");
        ...
        try {
          const data = await fetchGuestbook("otp_verify", {
            userMail: _liveOTP.email,
            otp: value,
          });
          _liveOTP.phase = "send";
          _liveOTP.email = "";
    ...
    document
      .getElementById("feed-otp-cancel-btn")
      ?.addEventListener("click", () => {
        cancelOTPVerify();
      });
  ...
  document
    .getElementById("feed-submit-btn")
    ?.addEventListener("click", async () => {
  ...
  document.getElementById("feed-edit-btn")?.addEventListener("click", () => {
    setModalPage("edit");
  });
  ...
  document
    .getElementById("feed-delete-btn")
    ?.addEventListener("click", async () => {
      setModalStatus("Deleting");
      try {
