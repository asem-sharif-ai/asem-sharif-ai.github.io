...
function buildLogCard(item, index, containerId) {
  ...
  const entityUI = item.entity
    ? item.entity_url
      ? `<a href='${item.entity_url}' target='_blank' rel='noopener noreferrer'>${item.entity}</a>`
      : item.entity
    : '';
  ...
  if (gallery && Array.isArray(gallery.content)) {
    const galleryList = gallery.content.filter(
      (src) => isImagePath(src) || isVideoPath(src),
    );
...
function buildSkillsCard(group, index) {
  ...
  if (highlight.length === 0 && content.length > 0) {
    ...
    col0Items.forEach((entry, i) => {
      rowItems.push({
        row: i + 1,
        col: 0,
        html: buildSlider(entry, i + 1, '1'),
      });
    });
    col1Items.forEach((entry, i) => {
      rowItems.push({
        row: i + 1,
        col: 1,
        html: buildSlider(entry, i + 1, '3 / 5'),
      });
    });
  } else {
    ...
    sldPlacements.forEach((p) =>
      rowItems.push({
        row: p.row,
        sort: 0,
        html: buildSlider(p.entry, p.row, p.isFull ? '1 / 5' : '1'),
      }),
    );
  ...
  card
    ...
    .addEventListener('click', () => toggleCard(cardId, collapseId));
  ...
  try {
    if (Array.isArray(skillsData) && skillsData.length > 0) {
      ...
      skillsData.forEach((group, index) => {
        container.appendChild(buildSkillsCard(group, index));
      });
      observeCards();
    } else {
      renderNoData('Skills', containerId);
    }
  } catch (e) {
...
let configData = null;
const validPages = ['education', 'experience', 'skills'];
...
async function runLogRouter() {
  try {
    ...
    if (!validPages.includes(page)) {
      const savedPage = localStorage.getItem(addresses.logActivePage);
